...
function finishWizard() {
  ...
  const starterIncome = state.setupComplete ? [] : createPaycheckTransactionsForMonth(elements.monthInput.value, {
  ...
  state = {
    ...
    transactions: state.setupComplete ? existingTransactions : starterIncome,
    setupComplete: true,
    setupProfile: {
  ...
  hideWizard();
  render();
  setMessage("Your budget is set up. Add your first expense whenever you are ready.");
}
...
function syncWizardControls() {
  elements.wizardPayAmountInput.value = wizard.payAmount;
...
function createWizardDraft(useCurrentState) {
  ...
  const currentIncome = profile.income || getMonthSummary(elements.monthInput?.value || currentMonthKey()).income || 4500;
  const payFrequency = profile.payFrequency || "biweekly";
  const payAmount = profile.payAmount || payAmountFromMonthly(currentIncome, payFrequency);
  ...
  return {
    ...
    payFrequency,
    payAmount: Math.max(0, Math.round(payAmount / 25) * 25),
    nextPayDate,
...
function normalizeSetupProfile(profile) {
  const payFrequency = profile?.payFrequency || "biweekly";
  const income = profile?.income || monthlyIncomeFromPay(profile?.payAmount || 2100, payFrequency);
  return {
    presetId: profile?.presetId || "single",
    income,
    payAmount: profile?.payAmount || payAmountFromMonthly(income, payFrequency),
    payFrequency,
...
function createPaycheckTransactionsForMonth(month, profileInput) {
  const profile = normalizeSetupProfile(profileInput);
  if (!profile.payAmount) return [];
  return getPaydaysForMonth(month, profile).map((date) =>
    transaction(
      date,
      "Income",
      "Salary",
      `${payFrequencies[profile.payFrequency]?.name || "Paycheck"} paycheck`,
      "Checking",
      profile.payAmount,
    ),
  );
