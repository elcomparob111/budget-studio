...
**UI status:** Apple + passkey buttons are hidden (`authProviders` in `sync-config.js` / `SyncConfig` on iOS). Google + email stay visible. Flip the flags when Apple/passkey are enabled in Supabase.
